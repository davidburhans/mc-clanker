...
async def delete_session_routing(session_id: uuid.UUID):
    ...
    db_manager = DatabaseManager.get_instance()
    with db_manager.session() as session:
        session.execute(text("DELETE FROM session_routing WHERE session_id = :session_id"), {"session_id": str(session_id)})
    return {"status": "ok"}
...
async def get_session_heartbeat(session_id: uuid.UUID):
    ...
    db_manager = DatabaseManager.get_instance()
    with db_manager.session() as session:
        result = session.execute(text("SELECT last_heartbeat FROM session_routing WHERE session_id = :session_id"), {"session_id": str(session_id)}).fetchone()
        if result is None:
            raise HTTPException(status_code=404, detail="Session not found")
