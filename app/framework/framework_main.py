...
def run_framework_loop():
    ...
    generator.load()
    print("DEBUG: Generator Registry loaded successfully.")
    ...
    try:
        ...
        while state.is_running:
            ...
            for model_id, engine in generator.models.items():
                ...
                import os
                desc = "No description"
                supported_families = ["Any"]
                # config/ is at the project root, two levels up from app/framework/
                config_path = os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(__file__))), "config", "models_config.json")
            ...
            if new_generation_requests:
                batch_results, _sr = generator.generate_batch(
                    new_generation_requests,
                    bpm=state.current_bpm,
