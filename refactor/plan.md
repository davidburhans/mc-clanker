...
> **STATUS (Phase 11 close-out): Phases 0–11 COMPLETE — Phase 11 (MixerController full port) LANDED in four units: U1 pinning, U2 additive promotion, U3 migration, U4 ctor-injection.** The full `E,F,I,UP006,UP007,UP045` ruff rule set is enforced and clean (0 errors); suite at **680 passed / 16 skipped**. E5 port-wiring: ALL FIVE ports (`ConductorPort`, `JobQueuePort`, `AudioFetchPort`, `AuditSinkPort`, `MixerController`) are now fully ctor-injected AND reached through their port abstraction — **R14 COMPLETE** (U4-await routed the last loop-reached path — `await_jobs` — through the injected port; only `AuditSinkPort.flush` remains `shows.py`-routed by design — zero behavior change, dual-ownership preserved via the shared module `_flush_lock`; see §1.3). Per-phase completion is tracked by the ✅ markers in §4 (Definition of Done).
...
class MixerController(Protocol):  # concrete Mixer satisfies structurally; public surface
    sample_rate: int  # PROMOTED (Phase 11 — U2 additive, U3 migrated, U4 ctor-injected)
    current_sample: int
...
**E5 port-wiring status (updated post-Phase-11 + U1-audio + U2-jobs + U3-audit + U4-await):** Of the five ports above, **ALL FIVE are now fully constructor-injected AND reached through their port abstraction — R14 COMPLETE.** — `AsyncFrameworkLoop.__init__` accepts `conductor=...` (commit `344a28f`, the E5 driving port), `audio=...` (U1-audio, an `AudioFetchPort | None` keyword whose default builds `GarageAudioAdapter(self._garage)` LAZILY via the `_audio` property, preserving the Gap-3 `_garage`-injection + lazy-env path), `mixer_factory=...` (Phase 11 U4, a `Callable[[], Mixer]` whose default IS the concrete `Mixer` class, so the real audio path is unchanged), `jobs=...` (U2-jobs, a `JobQueuePort | None` keyword whose default builds `PostgresJobQueueAdapter()` EAGERLY — its ctor is a no-op, the DB session opens lazily inside `submit` at call time; `_submit_job` now routes through `self._jobs.submit`, byte-for-byte unchanged), and `audit=...` (U3-audit, an `AuditSinkPort | None` keyword whose default builds `AuditAdapter()` EAGERLY — its ctor is a no-op, the DB session opens lazily inside `flush_recording_buffers` at call time, and the module-level `_flush_lock` is owned by the module functions, not the adapter; `_append_loop_audit` now routes through `self._audit.append_loop`, byte-for-byte unchanged). **U4-await LANDED (R14 complete):** the `JobQueuePort` await path is now routed through the injected port — the loop's `_await_jobs` delegate routes through `self._jobs.await_jobs` (the foreground `loop_steps._step_await_jobs_fetch` P8 + background `pregeneration.run_pregeneration` call sites both reach it; the Gap 4/5/6 characterization tests now patch `patch.object(loop, '_await_jobs')` instead of the module binding). The real await path is byte-for-byte unchanged (`PostgresJobQueueAdapter.await_jobs` wraps the module `await_jobs` → `wait_for_multiple_jobs`). **FLUSH is dual-owner as of rel-04** (REL-04): the loop now flushes past `AUDIT_FLUSH_THRESHOLD_ROWS` from P12 post-commit (`loop_steps._step_post_commit`), and `routes/shows.py` keeps the stop-show + lifespan-shutdown flushes; all flush paths serialize on the shared module `_flush_lock` (zero-loss: failed flushes re-queue in order).
R14 is now COMPLETE — every port path the loop reaches is routed through its injected port (the delegate-method seams `_submit_job` / `_await_jobs` / `_fetch_audio` / `_append_loop_audit` all delegate to the injected adapter). The only partial exception is `AuditSinkPort.flush`: dual-ownership as of rel-04 — the loop threshold-flushes from P12 (past `AUDIT_FLUSH_THRESHOLD_ROWS`) while `shows.py` keeps the stop-show + shutdown flushes; all flush paths serialize on the shared module `_flush_lock`. `MixerController` is now ctor-injected via a `mixer_factory` callable (Phase 11 U4); `AudioFetchPort` via an additive `audio=...` keyword seam (U1-audio); `JobQueuePort` submit + await via `jobs=...` (U2-jobs + U4-await) — see below.
...
| 11 | Full `MixerController` port *(LANDED)* | **very high** | — |
...
### Phase 11 — Full `MixerController` port ✅ COMPLETE (U1–U4 landed)
...
- **Status:** **COMPLETE — U1 (pinning) LANDED, U2 (additive promotion) LANDED, U3 (migration) LANDED, U4 (ctor-injection) LANDED.** U1 froze the four audio-path invariants a future port must preserve; U2 then promoted them to a public `MixerController`-satisfying surface **additively** (zero behavior change); U3 rewired the orchestrator off its private `Mixer` reach so it now calls `prime_loop` + `loop_position_seconds` exclusively, and the U1 pins evolved to follow the atomicity into its new home; U4 constructor-injects the `Mixer` behind the `MixerController` port via a `mixer_factory` callable (matching the Phase 7b `__init__(session_id, *, conductor, …)` seam), so the orchestrator takes the mixer as an injected dependency rather than constructing it inline via `Mixer()` in `start()`. All four units touch the real-time `_callback` audio path (brief-01 risk #5), but the invariants are pinned and the real audio path is byte-for-byte unchanged (default factory IS the concrete `Mixer` class).
...
- **U2 additive promotion — LANDED (branch `refactor/mixer-controller-surface`):** promoted the orchestrator→`Mixer` private reach to a public `MixerController`-satisfying surface **additively** — zero behavior change: the privates (`_add_track_internal`/`_ensure_stereo`/`_current_loop_duration`/`mixer.lock`) still coexist and the orchestrator still reaches them inline; migrating the orchestrator to *call* the new surface is U3. New public surface on concrete `Mixer`:
  ...
  U3 (rewire the orchestrator to call the new surface) has now **LANDED** — see below. U4 (constructor-inject the `Mixer`) has also **LANDED** — see below.
- **U3 migration — LANDED (branch `refactor/mixer-controller-migrate`):** the orchestrator now reaches `Mixer` ONLY via the `MixerController` surface — `_step_commit_to_mixer`'s loop-1 branch is a single `self.mixer.prime_loop(tracks_to_use, duration_samples=…)` call (U3a), and `_step_await_pregen`'s P13 boundary read is a single `self.mixer.loop_position_seconds()` call (U3b). Zero `with self.mixer.lock:` / `_add_track_internal` / `_ensure_stereo` / `_current_loop_duration` reach remains in `loop_steps.py` or `loop_orchestrator.py`; that private reach now lives solely inside `Mixer`. **The U1 pins EVOLVED (retargeted), never deleted, so the atomicity invariant moved WITH the code:** invariant 1 (P10 atomicity) is now TWO pins — `test_step_commit_to_mixer_loop1_is_single_prime_loop_call` asserts the loop-1 branch is a single `prime_loop` delegation with zero `mixer.lock` in the orchestrator, and `test_prime_loop_is_single_lock_batch` pins the SAME 4-statement sync `with self.lock:` batch inside `Mixer.prime_loop`; invariant 3 (P13) is now `test_p13_delegates_boundary_read_no_mixer_lock` (asserts the `loop_position_seconds()` delegation + zero `mixer.lock`). A new permanent source-guard, `test_orchestrator_has_no_private_mixer_reach` (AST scan of `loop_steps.py` + `loop_orchestrator.py`), makes the zero-private-reach migration non-regressable: any reintroduction of `_add_track_internal`/`_ensure_stereo`/`_current_loop_duration` or `with self.mixer.lock:` fails fast. `test_mixer_controller_characterization.py::test_p10_loop1_holds_lock_during_every_batch_write` now drives the delegation through `_LockSpyMixer.prime_loop` and asserts `prime_loop_calls == 1`. Invariants 2 (lock-held) + 4 (crossfade round-trip) are unchanged. The lock acquired inside both methods is the SAME `threading.Lock` the daemon `_callback` holds during the crossfade, so dual-lock timing and the crossfade round-trip are preserved.
  U4 (constructor-inject the `Mixer` behind the `MixerController` port — `__init__(session_id, *, conductor, mixer, …)` matching the Phase 7b port-wiring seam, so the orchestrator takes the mixer as an injected dependency rather than constructing it inline via `Mixer()` in `start()`) is now **LANDED** — see below.
...
### Post-Phase-11 — E5 port-wiring continuation (U1-audio, U2-jobs, U3-audit) ✅ LANDED
...
- **U2-jobs — JobQueuePort submit ctor-injection — LANDED (branch `refactor/jobs-port-inject`):** a post-Phase-11 continuation of the E5 DI seam — `AsyncFrameworkLoop.__init__` now also accepts `jobs: JobQueuePort | None = None` (mirroring the Phase 11 U4 `mixer_factory` + U1-audio `audio` seams). The default builds the concrete `PostgresJobQueueAdapter()` EAGERLY (its ctor is a no-op — the DB session opens lazily inside `submit_generator_job` at call time, so unlike the audio port there is no lazy-env / Gap-3 path to preserve), and `_submit_job` now routes through `self._jobs.submit` instead of the module function directly, so callers that omit the kwarg (e.g. `app_ui.py`, the test harness) are byte-for-byte unchanged. TDD-red suite `tests/test_jobs_injection.py` pins: (1) omitting `jobs` yields the concrete `PostgresJobQueueAdapter`, stored EAGERLY; (2) the default adapter structurally satisfies `JobQueuePort`; (3) an injected `JobQueuePort` fake is stored verbatim and reached by `_submit_job` (the real `submit_generator_job` is NOT called — real DI); (4) the pre-existing `loop._submit_job = AsyncMock` / `loop._jobs = <fake>` direct-assignment harness still works (the seam is additive, never breaks it). NOTE: only the **submit** path is routed through the injected port; **`await_jobs` routing remains deferred to U4** — the foreground `_run_loop` + background `_pre_generate_next_loop` still call `wait_for_multiple_jobs` via the `framework_main_async` module binding (the Gap 4/5/6 characterization tests monkeypatch THAT binding). The `JobQueuePort` Protocol docstring in `ports.py` now notes the submit ctor-injection landed.

- **U3-audit — AuditSinkPort append ctor-injection — LANDED (branch `refactor/audit-port-inject`):** a post-Phase-11 continuation of the E5 DI seam — `AsyncFrameworkLoop.__init__` now also accepts `audit: AuditSinkPort | None = None` (mirroring the Phase 11 U4 `mixer_factory` + U1-audio `audio` + U2-jobs `jobs` seams). The default builds the concrete `AuditAdapter()` EAGERLY (its ctor is a no-op — the DB session opens lazily inside `flush_recording_buffers` at call time, and the module-level `_flush_lock` is owned by the module functions, not the adapter, so unlike the audio port there is no lazy-env / Gap-3 path to preserve), and `_append_loop_audit` now routes through `self._audit.append_loop` instead of the module function directly, so callers that omit the kwarg (e.g. `app_ui.py`, the test harness) are byte-for-byte unchanged. The new `AuditAdapter` class (`audit_recording.py`) wraps the module `append_loop_audit` + `flush_recording_buffers` and takes NO lock of its own — the `_flush_lock` + `state.lock` semantics live in the module functions (B13). TDD-red suite `tests/test_audit_injection.py` pins: (1) omitting `audit` yields the concrete `AuditAdapter`, stored EAGERLY; (2) the default adapter structurally satisfies `AuditSinkPort`; (3) an injected `AuditSinkPort` fake is stored verbatim and reached by `_append_loop_audit` (the real `append_loop_audit` module function is NOT called — real DI); (4) the pre-existing `loop._append_loop_audit = AsyncMock` / `loop._audit = <fake>` direct-assignment harness still works (the seam is additive, never breaks it). NOTE: only the **append** path is routed through the injected port; **`flush` routing remains deferred to U4** — `shows.py` still calls the module `flush_recording_buffers` directly (dual-ownership preserved). The `AuditSinkPort` Protocol docstring in `ports.py` now notes the append ctor-injection landed.

- **U4-await — JobQueuePort await routing through the orchestrator — LANDED (branch `refactor/ports-await-rewire`):** closes the Phase 7b deferred landmine and **completes R14** — all five ports are now ctor-injected AND reached through their port abstraction. A new `AsyncFrameworkLoop._await_jobs` delegate (mirroring `_submit_job`/`_fetch_audio`/`_append_loop_audit`) routes through `self._jobs.await_jobs`; the two direct `wait_for_multiple_jobs` call sites (`loop_steps._step_await_jobs_fetch` P8 + `pregeneration.run_pregeneration`) now call `self._await_jobs` / `loop._await_jobs` instead, and their dead `from app.job_waiter import wait_for_multiple_jobs` imports are removed. The real await path is byte-for-byte unchanged (`PostgresJobQueueAdapter.await_jobs` still wraps the module `await_jobs` → `wait_for_multiple_jobs`). The double-binding test patches (6 sites across `test_pregeneration_divergence.py` + `test_framework_characterization.py`) migrated to `patch.object(loop, '_await_jobs')` / direct `loop._await_jobs = AsyncMock` assignment — canned return shapes preserved exactly so the foreground-vs-pregen `state.cache_stem` divergence (Gap 4/5/6) is unchanged. TDD-red suite `tests/test_jobs_await_injection.py` pins: (a) `_await_jobs` delegates to `self._jobs.await_jobs` with identical kwargs; (b) an injected fake `JobQueuePort` controls the result AND the real `wait_for_multiple_jobs` is NOT called (the landmine pin); (c) the pre-existing `loop._await_jobs = AsyncMock` direct-assignment harness still works. **FLUSH (the §7 caveat):** `AuditSinkPort.flush` is structurally complete (the port declares it; `AuditAdapter.flush()` from U3 delegates to `flush_recording_buffers`) but is deliberately NOT newly routed into the loop — the loop never calls flush today (only `app/routes/shows.py:319` does). U4 adds zero loop-side flush calls (zero behavior change, matching the governing "no scope creep" philosophy) — SUPERSEDED by rel-04 (REL-04 remediation): the loop now threshold-flushes from P12 past `AUDIT_FLUSH_THRESHOLD_ROWS`; `shows.py` keeps stop-show + shutdown flushes; dual-ownership preserved (shared module-level `_flush_lock` serializes `flush_recording_buffers` across callers).
...
| R5 | MixerPort dual-lock deadlock (brief-01 #5) | critical | Phase 11 (deferred → guarded → RESOLVED/MIGRATED) | Orchestrator no longer reaches `Mixer` privates (U3 — `test_orchestrator_has_no_private_mixer_reach` AST source-guard). P10/P13 audio behavior unchanged: U3 rewired call sites to `prime_loop`/`loop_position_seconds` byte-for-byte. Atomicity PINNED (U1) + non-regressable (source-guarded). `Mixer` ctor-injected via `mixer_factory` (U4) — default factory IS the concrete `Mixer` class, so zero audible change. **Status: deferred → guarded → RESOLVED/MIGRATED. Phase 11: U1–U4 LANDED.** |
...
| R14 | Ports non-injectable (internal-only construction) → E5/CLAUDE.md DI goal silently unmet (round-1) | medium | Phase 7b + Phase 11 U4 + U1-audio + U2-jobs + U3-audit + U4-await | `ConductorPort` ctor-injected (Phase 7b); `MixerController` ctor-injected via `mixer_factory` callable (Phase 11 U4); `AudioFetchPort` ctor-injected via `audio=...` (U1-audio); `JobQueuePort` submit + await ctor-injected + routed via `jobs=...` (U2-jobs + U4-await); `AuditSinkPort` append ctor-injected + routed via `audit=...` (U3-audit; flush remains `shows.py`-routed by design). ALL FIVE ports ctor-injected AND reached through the port abstraction. `test_loop_accepts_injected_fake_ports` + `tests/test_mixer_injection.py` + `tests/test_audio_injection.py` + `tests/test_jobs_injection.py` + `tests/test_jobs_await_injection.py` + `tests/test_audit_injection.py`. **DONE (R14 COMPLETE).** |

---

### Summary

- **Phases:** 11 core (0–10) + 1 (Phase 11, optional — **LANDED**). Each commit keeps the suite green.
