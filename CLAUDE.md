...
| `loop_orchestrator.py` | `AsyncFrameworkLoop` | Async DJ set orchestrator — lifecycle (`__init__`/`start`/`stop`) + the thin `_run_loop` driver only; ctor-injects `ConductorPort` + `MixerController` (via `mixer_factory`); loop-failure semantics (REL-18/19): failed iterations retry on an escalated ±25 %-jitter backoff (`loop_retry_backoff_delay`, first failure keeps the flat 2 s, cap 30 s, ladder resets on a clean pass) and the conductor call is skipped once 3 consecutive job submits fail (`LOOP_CONDUCTOR_SKIP_AFTER_SUBMIT_FAILURES`, foreground + pregen gated; FU-2: the streak resets ONLY on a successful submit — the once-per-loop `pending_depth()` read probe gates a one-shot write canary through the `_submit_job` seam so a read-only hot standby can never re-enable the LLM call); pregen results carry a spawn-time `pregen_epoch` stamp and P3 bumps the monotonic counter on every `should_reset` consumption, so a pre-reset in-flight result is rejected when post-reset indices revisit (FU-2/rel-02 residual); a startup failure flips only `state.is_running` — the `trigger_shutdown()` kill switch stays reserved for process shutdown |
| `loop_delegates.py` | `_LoopDelegates` | Adapter-delegates mixin (FU-2 pure move from `loop_orchestrator.py` to stay under the 500-LOC rule): `_build_prompt`/`_submit_job`/`_await_jobs`/`_abandon_jobs`/`_pending_depth`/`_fetch_audio`/`_append_loop_audit`/`_pre_generate_next_loop` + the FU-2 `_canary_submit` write probe (oldest-active-stem payload via `_canary_payload`, best-effort abandoned; the streak's only reset path is a successful submit through the seam). First in `AsyncFrameworkLoop`'s MRO so these win over `_LoopSteps`' raising stubs; `patch.object(loop, '_submit_job')` etc. resolve unchanged |
...
| `app/db.py` | SQLAlchemy DatabaseManager singleton (thread-safe); REL-09: PG engines get pool_pre_ping/pool_recycle for every PG dialect + libpq connect/statement timeouts and TCP keepalives (FU-5: connect_args gated on the PG DRIVER via `_pg_connect_args` — `postgresql://`/`+psycopg2://` keep the full set, a non-libpq driver like asyncpg builds with `connect_args == {}` + a naming warning; SQLite paths unchanged) |
...
| `app/worker.py` | Async job processor (separate container; FU-3: breaker counter surfaced on /health, timeout re-wrap `GenerationIoTimeout` keeps pipeline I/O timeouts off the breaker, `_generate_and_upload` is a provenance wrapper over `_run_generation_pipeline`; FU-5 DECISION: 498/500 headroom stays — under the ceiling, pinned by `test_worker_fu3.py` S1, no further slice. Candidate seams if it ever grows: /health+get_stats pair, VRAM-eviction pair, CLI tail; do NOT move the breaker block — `GENERATION_TIMEOUT_SECONDS` is module-attr-patched in 7 test sites) |
...
| `test_fu5_cosmetics.py` | FU-5: fanout split LOC pin (both files <500) + sentinel identity + old-import-path resolution (zero test edits); reasoning_stats 13-helper annotation pin; audit completion banner pin (banner above the preserved verdict) — plus `tests/test_db.py::TestLibpqDriverGatingFu5` for the driver-gated connect_args (D1 asyncpg bare / D2 psycopg2 exact) |
